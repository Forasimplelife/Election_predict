...
try:
    ...
    if actual_smd_input and actual_pr_input:
        ...
        print(f"{'比例代表':<15} {PREDICTED_PR:>10} {actual_pr:>10} {error_pr:>+10} {100-error_pct_pr:>9.1f}%")
        ...
        ax1.bar(x - width/2, predicted, width, label='预测', color='#6c757d', alpha=0.8)
        ax1.bar(x + width/2, actual, width, label='实际', color='#dc3545', alpha=0.8)
        ax1.set_ylabel('席次')
        ...
        pd.DataFrame([validation_data]).to_csv('results/validation_results.csv', index=False)
